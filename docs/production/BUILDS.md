...
Актуальная сборка после отзыва о размере героя: **0.4.3-closer-camera**, Android код 8, дистанция камеры 3,0 м. Windows проверен запуском, S23 обновлён и сцена отображается. [Протокол и хэши](CLOSER_CAMERA_CHECKS.md). Пакеты 0.4.1 сохранены локально в `.tools/builds/0.4.1-running-redraw/`; файлы в стандартных `.tools/builds/windows` и `.tools/builds/android` теперь относятся к 0.4.3.
