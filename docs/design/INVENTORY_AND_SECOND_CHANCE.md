...
```mermaid
flowchart TD
    A[Герой в сознании] -->|Первый нокаут| B[Падает на четвереньки]
    B --> C[Без сознания]
    C -->|Спустя время| D[Пробуждение: минимум здоровья]
    D -->|Восстановился по выбранным правилам| A
    D -->|Повторный нокаут в опасный период| E[Смерть]
```
...
| DOWN-01 | Нокаут → четвереньки → обморок → пробуждение с минимумом здоровья → восстановление либо смерть при повторном нокауте | Входящий урон COMBAT-01/02, базовые здоровье/раны WOUND-01, VIS-02, REST-01, TIME-01 и SAVE-01. Проверить обе ветви, границу периода риска, повторный урон и загрузку в каждом состоянии |
...
- Базовая вместимость, единицы (ячейки/вес/объём), прирост за уровень и силу. Два положительных вклада подтверждены; коэффициенты и соотношение не назначены.
- Размер кошелька, допустимые мелочи и стопки; входят ли монеты из текущего отдельного баланса и физическая карта. Не объявлять защищёнными все деньги/квестовые вещи только по категории.
- Перенос содержимого старых рюкзака/мешочка, судьба самих предметов и случай переполнения. Миграция обязана сохранять вещи и устойчивые ссылки, не выбрасывать излишек молча.
