...
Исследование от 22 сентября 2026 года. **Документ для обсуждения и следующих ограниченных проб; игровая реализация этим изменением не добавлена.**

Владелец положительно оценил пробу волка и стражника 0.17.0, попросил обсудить пространственные эффекты входящей атаки/блока, hitstop и изучить Fighting Game Frame Data System. Это подтверждает интерес к направлению, но не утверждает предложенные ниже числа, комбо, правила оглушения или окончательный художественный стиль. Ошибка светлой палитры обученного в блоке остаётся открытой.
...
Диаграмма — предложение структуры, не точная схема нынешних классов. Формат данных выбирается отдельной ограниченной задачей Ollama; полноценный редактор атак сразу не требуется.
...
| --- | --- | --- |
| Hitstop обычного попадания | Сравнить 0, 40–60 мс | Не реализовано и не утверждено |
| Акцент идеального блока | Сравнить 70–90 мс с обычным попаданием | Не реализовано и не утверждено |
...
Предложенная последовательность, а не объявление новой готовой фичи:

1. Согласовать направление по этому конспекту. Ошибку светлого блока держать отдельной правкой; новое предложение её не закрывает.
2. Одна проба на существующих врагах: эффект входящей атаки, различимые hit/block/perfect и короткий hitstop. Сохранить нынешние дальности, одиночный контакт и окно блока, чтобы оценивать именно обратную связь.
...
Основные проверки проектирует и выполняет Codex, игровую реализацию и исправления пишет Ollama. Каждая ограниченная задача — новая ветка от актуального dev с PR. Предлагаемые сценарии:
...
Изменение касается только Markdown. Проверяются относительные ссылки, наличие обеих видеоссылок и Reddit, формулы примера, отделение фактов от предложений, согласованность с памятью/планом и отсутствие изменений игровых файлов. Игровые тесты, новые сборки и установка на телефон для этого документа не выполняются.
