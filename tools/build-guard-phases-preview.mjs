...
const name = validation ? 'phases-validation' : 'phases-preview';
...
const title = validation ? 'Guard Phases Validation' : 'Guard Attack Phases';
const version = '0.18.0-guard-phases';
const packageId = validation ? 'org.ashbound.guardphasesvalidation' : 'org.ashbound.fistqa';
for (const file of ['project.godot','export_presets.cfg']) {
  ...
  if (file === 'project.godot') {
    ...
    data = data.replaceAll('export_files=PackedStringArray(',`export_files=PackedStringArray(${extra}, `)
      ...
      .replace(/version\/code=\d+/, 'version/code=40')
...
const windows = path.join(root,'.tools/builds/phases-preview/AshBound-Guard-Phases.exe');
...
if (!validation) {
  const log = path.join(root,'.tools/phases-preview-package-smoke.log');
  ...
  if (/SCRIPT ERROR:|^ERROR:/m.test(output) || !output.includes('ASHBOUND_GUARD_PHASES_READY')) throw Error(log);
