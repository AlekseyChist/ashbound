...
const stage=path.join(root,'.tools/export-staging/side-gait-validation');
const godot=path.join(baseline,'.tools/godot/Godot_v4.7.2-stable_win64_console.exe');
const out=path.join(root,'.tools/side-gait-checks');fs.mkdirSync(out,{recursive:true});
...
run('gait-render',[...graphics,'res://scripts/tools/validate_side_gait.tscn'],'ASHBOUND_SIDE_GAIT_OK groups=16');
const user=path.join(process.env.APPDATA,'AshBound_Forest_Route_QA');
for(const f of fs.readdirSync(user))if(f==='side-gait-results.json'||/^gait-.*\.png$/.test(f))fs.copyFileSync(path.join(user,f),path.join(out,f));
...
console.log('ASHBOUND_SIDE_GAIT_CHECKS_OK');
