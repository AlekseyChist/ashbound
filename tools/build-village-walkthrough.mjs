...
const qa=args.includes('--validate'),name=qa?'village-walkthrough-validation':'village-walkthrough-preview';
const stage=path.join(root,'.tools/export-staging',name),out=path.join(root,'.tools/village-walkthrough-checks');
...
if(!args.includes('--stage-only')){
 ...
 const windows=path.join(root,'.tools/builds/village-walkthrough/AshBound-Village-Houses.exe');
