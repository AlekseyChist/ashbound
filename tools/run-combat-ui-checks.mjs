...
const stage=path.join(root,'.tools/export-staging',uiBook?'ui-book-validation':hudInput?'hud-input-validation':'combat-ui-validation');
...
const suites=[...(uiBook?[['ui_book',8,'UI_BOOK']]:[]),...(hudInput?[['hud_input',9,'HUD_INPUT']]:[]),['combat_ui',12,'COMBAT_UI'],...(args.includes('--regressions')?[['combat_dodge',10,'COMBAT_DODGE'],['guard_attack_phases',8,'GUARD_PHASES'],['painted_combat',8,'PAINTED_COMBAT'],['combat_feedback',9,'COMBAT_FEEDBACK']]:[])];
for(const [scene,count,marker] of args.includes('--campaign-only')?[]:suites){
...
if(hudInput && args.includes('--regressions') && !args.includes('--render')){
 for(const [name,marker,extra] of [
  ...
  fs.copyFileSync(path.join(root,'scripts/tools',`validate_${name}.gd`),path.join(stage,'scripts/tools',`validate_${name}.gd`));
