// Codex packaging: isolated enemy preview or native independent QA. Main build unchanged.
...
const name = validation ? 'enemy-validation' : 'enemy-preview';
...
const title = validation ? 'Enemy Validation' : 'Enemy Preview';
const version = '0.17.0-enemy-preview';
const packageId = validation ? 'org.ashbound.enemyvalidation' : 'org.ashbound.fistqa';
for (const file of ['project.godot','export_presets.cfg']) {
  ...
  if (file === 'project.godot') {
    ...
    data = data.replaceAll('export_files=PackedStringArray(',`export_files=PackedStringArray(${extra}, `)
      ...
      .replace(/version\/code=\d+/, 'version/code=35')
...
const windows = path.join(root,'.tools/builds/enemy-preview/AshBound-Enemy-Preview.exe');
...
if (!validation) {
  const log = path.join(root,'.tools/enemy-preview-package-smoke.log');
  ...
  if (/SCRIPT ERROR:|^ERROR:/m.test(output) || !output.includes('ASHBOUND_CORNER_ENEMIES_READY')) throw Error(log);
  console.log('ASHBOUND_ENEMY_PACKAGE_SMOKE_OK');
}
console.log(android);
