...
for(const script of ['combat_dodge_player','combat_dodge_session','validate_combat_dodge'])
 run('dodge-parse-'+script,['--headless','--check-only','--script',`res://scripts/tools/${script}.gd`],null,20000);
for(const render of process.argv.includes('--headless-only') ? [false] : [false,true]){
 for(const [scene,marker] of [['combat_dodge','ASHBOUND_COMBAT_DODGE_OK groups=10'],['guard_attack_phases','ASHBOUND_GUARD_PHASES_OK groups=8'],['painted_combat','ASHBOUND_PAINTED_COMBAT_OK groups=8'],['combat_feedback','ASHBOUND_COMBAT_FEEDBACK_OK groups=9']])
  run(`dodge-${scene}-${render?'render':'core'}`,[...(render?['--windowed','--resolution','1600x900','--position','-10000,-10000']:['--headless']),`res://scripts/tools/validate_${scene}.tscn`],marker);
}
console.log('ASHBOUND_COMBAT_DODGE_CHECKS_OK');
