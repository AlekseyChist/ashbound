// Codex QA: run the same independent display suite in an isolated Android app.
import fs from 'node:fs';
import path from 'node:path';
import {execFileSync} from 'node:child_process';

const root = path.resolve(import.meta.dirname, '..');
const stage = path.join(root, '.tools/export-staging/adaptive-display-qa');
const output = path.join(root, '.tools/builds/android/ashbound-adaptive-display-qa.apk');
...
const script = 'scripts/tools/adaptive_android_validation.gd';
const scene = 'scripts/tools/adaptive_android_validation.tscn';
const test = fs.readFileSync(path.join(root, 'scripts/tools/validate_adaptive_display.gd'), 'utf8')
...
for (const file of ['project.godot', 'export_presets.cfg']) {
  ...
  if (file === 'project.godot') {
    data = data.replace(/run\/main_scene="[^"]+"/, `run/main_scene="res://${scene}"`)
      .replace('config/name="ASHBOUND"', 'config/name="ASHBOUND Display QA"')
    ...
    data = data.replaceAll('export_files=PackedStringArray(', `export_files=PackedStringArray(${resources.map(f => `"res://${f}"`).join(', ')}, `)
      .replaceAll('org.ashbound.courtyard', 'org.ashbound.displayqa')
      .replaceAll('package/name="AshBound Courtyard"', 'package/name="AshBound Display QA"');
  }
...
for (const [step, args] of [['import', ['--editor', '--import', '--quit']],
  ...
  const log = path.join(root, `.tools/adaptive-display-qa-${step}.log`);
  ...
  console.log(`ADAPTIVE_DISPLAY_QA_${step.toUpperCase()}_OK`);
}
console.log(output);
