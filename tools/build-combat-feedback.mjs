...
const resources = [scene, 'scripts/tools/combat_feedback_sandbox.tscn', 'scripts/combat/combat_swing_trails.gd', ...['fx','player','session','sandbox'].map(id=>(fs.existsSync(path.join(root,'scripts/combat/combat_feedback_'+id+'.gd'))?'scripts/combat/':'scripts/tools/')+'combat_feedback_'+id+'.gd'), 'scripts/tools/corner_enemy_sandbox.tscn', ...['actor','visual','session','level','toolbar','preview_toolbar'].map(id=>(fs.existsSync(path.join(root,'scripts/combat/corner_enemy_'+id+'.gd'))?'scripts/combat/':'scripts/tools/')+'corner_enemy_'+id+'.gd'), ...['wolf','guard'].map(id=>'assets/characters/courtyard/enemy-preview/'+id+'_frames.tres'), 'scripts/tools/fist_defense_sandbox.tscn',
