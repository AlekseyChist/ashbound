...
function run(name,args,marker,timeout=120000){
 ...
 if(failure || /SCRIPT ERROR:|^ERROR:|FEEDBACK_FAIL:|DEFENSE_POSES_FAIL:|INCOMPLETE|TIMEOUT/m.test(out) || (marker && !out.includes(marker))){console.log(out.slice(-18000));throw Error(log);}
 console.log('PASS '+name);
}
// Sandbox depends on project autoloads, so compile it through its real scene below.
for(const script of ['correct_trained_guard_palette','validate_guard_palette'])
...
for(const render of process.argv.includes('--headless-only') ? [false] : [false,true]){
 for(const [scene,marker] of [['guard_palette','ASHBOUND_GUARD_PALETTE_RUNTIME_OK groups=8'],['painted_combat','ASHBOUND_PAINTED_COMBAT_OK groups=8'],['fist_defense_poses','ASHBOUND_FIST_DEFENSE_POSES_OK groups=6']])
...
console.log('ASHBOUND_GUARD_PALETTE_CHECKS_OK');
