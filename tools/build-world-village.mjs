// WORLD-VILLAGE-01: the starter village on the world map as a separate phone package.
// Never changes the source project's launch scene.
...
const qa=args.includes('--validate'),name=qa?'world-village-validation':'world-village-preview';
const stage=path.join(root,'.tools/export-staging',name),out=path.join(root,'.tools/world-village-checks');
...
let project=fs.readFileSync(path.join(stage,'project.godot'),'utf8')
 ...
 .replace('[application]',`[application]\nconfig/use_custom_user_dir=true\nconfig/custom_user_dir_name="AshBound_World${qa?'_QA':''}"`);
fs.writeFileSync(path.join(stage,'project.godot'),project);
const resources=[scene,'scenes/world/world.tscn','scenes/world/village_settlement.tscn','scenes/world/village_walkthrough.tscn','scenes/world/player_rig.tscn','scenes/world/village_pocket_menu.tscn','scripts/world/player_rig.gd','data/quests/village_lesson.tres','data/quests/forest_inn_keeper.tres','scenes/buildings/watchtower.tscn','scenes/courtyard/resident.tscn','scenes/courtyard/props/woodpile.tscn','scenes/courtyard/props/dummy.tscn','assets/characters/courtyard/watchman_frames.tres','assets/characters/courtyard/innkeeper_frames.tres','scripts/courtyard/interaction_point.gd','scripts/courtyard/courtyard_resident.gd','assets/buildings/ashbound/courtyard_well.glb','assets/environment/medieval_kit/detail-barrel.glb','assets/environment/medieval_kit/detail-crate.glb','assets/environment/medieval_kit/fence-wood.glb',...files.filter(f=>f.startsWith('scripts/world/village_')||f.startsWith('scripts/world/world')||f.startsWith('assets/environment/village-forest-v1/')||f.startsWith('assets/environment/forest-floor-v1/')||(f.startsWith('assets/environment/village-house-materials-v1/')&&f.endsWith('.jpg'))||(f.startsWith('assets/environment/village-surfaces-v1/')&&f.endsWith('.jpg'))||f.startsWith('assets/shaders/village_')||f.startsWith('assets/buildings/forest-village-v1/')||f.startsWith('assets/buildings/forest-inn-v1/')||f.startsWith('assets/props/tavern-v1/')||f.startsWith('scripts/world/inn_')||f.startsWith('assets/characters/world-graybox-v1/')).filter(f=>!f.endsWith('.import')&&!f.endsWith('.uid'))];
...
let presets=fs.readFileSync(path.join(stage,'export_presets.cfg'),'utf8')
 ...
 .replaceAll('include_filter="localization/*.po,assets/ui/fonts/OFL.txt"','include_filter="localization/*.po,assets/ui/fonts/OFL.txt,assets/world/*.json,assets/world/graybox-v1/*"')
...
if(!args.includes('--stage-only')){
 ...
 const windows=path.join(root,'.tools/builds/world-village',version,'AshBound-World.exe');
