...
Локальной Qwen поручаем простые ограниченные изменения; сложный ввод, взаимодействие нескольких систем и трудные исправления выполняет Codex напрямую (D-047). Отдельное правило Blender сохраняется. Основные тесты готовит, пишет/исправляет и запускает Codex; не передавать ключевые проверки мосту Ollama. После повторного неуспеха Qwen разбирать причину и передавать трудную часть Codex вместо цепочки перепоручений. [Полные правила](../AGENTS.md), выбор исполнителя и авторство фиксируются в карточке.
...
Для полностью новых файлов можно добавить `--create-only` вместе с `--write --files-only`: модели доступно только создание файлов, перезапись существующего запрещена. Это сокращает ненужное чтение старых прототипов. Для исправлений этот флаг не подходит.

`--max-writes N` возвращает управление после N успешных полных записей `write_file`, чтобы координатор проверил результат до следующей генерации. Это ограничитель объёма, а не подтверждение выполнения задания: число файлов, содержание и запуск всё равно проверяются. Точечные `replace_text` в этот счётчик не входят.

Для узкой правки можно передать `--allow-file scripts/courtyard/courtyard_level.gd` (повторяемый флаг, только с `--files-only`). В таком запуске мост разрешает чтение и запись только перечисленных файлов и убирает поиск по проекту. Это помогает не распылять контекст локальной модели на соседние задачи; результат по-прежнему нужно проверять.

Для локализации мост также читает и записывает `.po`/`.pot` и допускает точечную настройку корневого `export_presets.cfg`. Передавать конкретные каталоги/конфигурацию через `--allow-file`; это не открывает произвольные `.cfg` для записи. Каталоги проверять через `node tools/validate-localization-catalogs.mjs` и реальный экспорт, как описано в [протоколе LOC-01A](../docs/production/LOCALIZATION_FOUNDATION_CHECKS.md).

После Android-экспорта запускать `tools/validate-courtyard-apk.ps1 -ApkPath .tools/builds/android/ashbound-courtyard.apk`: проверка ищет скрипты автозагрузки и их явные зависимости в APK и запрещает включение QA-сцен/скриптов. Она воспроизводит найденный в INV-01B пропуск компонента хранения при экспорте выбранных сцен. Проверка состава не заменяет запуск готового EXE/APK и чтение журнала — код 0 и появление двора при ошибке автозагрузки не считаются успехом.
