...
const name = validation ? 'dodge-validation' : 'dodge-preview';
...
const scene = validation ? 'scripts/tools/validate_combat_dodge.tscn' : 'scripts/tools/combat_dodge_sandbox.tscn';
const resources = [scene, 'scripts/tools/combat_dodge_sandbox.tscn', ...['player','session','toolbar','sandbox'].map(id=>'scripts/tools/combat_dodge_'+id+'.gd'), 'scripts/tools/guard_phases_sandbox.tscn', 'scripts/tools/guard_phases_sandbox.gd', 'scripts/tools/guard_phases_session.gd', 'scripts/combat/frame_guard_actor.gd', 'scripts/combat/attack_frame_data.gd', 'assets/combat/guard_unarmed_v1.tres', 'scripts/tools/painted_combat_sandbox.tscn', 'scripts/combat/painted_effect_pool.gd', ...['fx','trails','session','sandbox'].map(id=>'scripts/tools/painted_combat_'+id+'.gd'), ...['hit','block','perfect_block','windup','swing'].map(id=>'assets/vfx/painted-combat-v1/'+id+'.png'), 'scripts/tools/combat_feedback_sandbox.tscn', 'scripts/combat/combat_swing_trails.gd', 'scripts/tools/validate_combat_feedback.gd', ...['fx','player','session','sandbox'].map(id=>'scripts/tools/combat_feedback_'+id+'.gd'), 'scripts/tools/corner_enemy_sandbox.tscn', ...['actor','visual','session','level','toolbar','preview_toolbar'].map(id=>'scripts/tools/corner_enemy_'+id+'.gd'), ...['wolf','guard'].map(id=>'assets/characters/courtyard/enemy-preview/'+id+'_frames.tres'), 'scripts/tools/fist_defense_sandbox.tscn',
...
const title = validation ? 'Combat Dodge Validation' : 'Combat Dodge';
const version = '0.18.1-combat-dodge';
const packageId = validation ? 'org.ashbound.dodgevalidation' : 'org.ashbound.fistqa';
for (const file of ['project.godot','export_presets.cfg']) {
  ...
  if (file === 'project.godot') {
    ...
    data = data.replaceAll('export_files=PackedStringArray(',`export_files=PackedStringArray(${extra}, `)
      ...
      .replace(/version\/code=\d+/, 'version/code=41')
...
const windows = path.join(root,'.tools/builds/dodge-preview/AshBound-Combat-Dodge.exe');
...
if (!validation) {
  const log = path.join(root,'.tools/dodge-preview-package-smoke.log');
  ...
  if (/SCRIPT ERROR:|^ERROR:/m.test(output) || !output.includes('ASHBOUND_COMBAT_DODGE_READY')) throw Error(log);
  console.log('ASHBOUND_DODGE_PACKAGE_SMOKE_OK');
}
console.log(android);
