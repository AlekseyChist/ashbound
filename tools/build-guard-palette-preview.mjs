...
const name = validation ? 'palette-validation' : 'palette-preview';
...
const title = validation ? 'Guard Palette Validation' : 'Guard Palette';
const version = '0.17.4-guard-palette';
const packageId = validation ? 'org.ashbound.guardpalettevalidation' : 'org.ashbound.fistqa';
for (const file of ['project.godot','export_presets.cfg']) {
  ...
  if (file === 'project.godot') {
    ...
    data = data.replaceAll('export_files=PackedStringArray(',`export_files=PackedStringArray(${extra}, `)
      ...
      .replace(/version\/code=\d+/, 'version/code=39')
...
const windows = path.join(root,'.tools/builds/palette-preview/AshBound-Guard-Palette.exe');
...
if (!validation) {
  const log = path.join(root,'.tools/palette-preview-package-smoke.log');
