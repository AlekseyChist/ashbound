...
func _ready() -> void:
	...
	get_tree().create_timer(90).timeout.connect(func():
		push_error("PAINTED_MOTION_TIMEOUT")
		get_tree().quit(1))
	_run.call_deferred()
...
func _run() -> void:
		...
		for clip in [&"walk", &"walk_side", &"run_side"]:
			...
			for i in range(8):
				...
				if texture != null:
					...
					check(("painted-motion-v2" if frames == Bare else "side-gait-v1") in texture.atlas.resource_path, "correct new/legacy atlas used")
	...
	check(is_equal_approx(float(Bare.get_meta("pixel_size_run_side")), float(Pack.get_meta("pixel_size_run_side"))), "legacy equipment scale remains compatible")
	await _transitions()
	await _capture_poses()
	var result := {"platform": OS.get_name(), "groups": groups, "traces": traces, "failures": failures}
	var file := FileAccess.open("user://painted-motion-results.json", FileAccess.WRITE)
	file.store_string(JSON.stringify(result, "\t"))
	file.close()
	if failures.is_empty():
		print("ASHBOUND_PAINTED_MOTION_OK groups=%d" % groups)
	else:
		for error in failures:
			push_error("PAINTED_MOTION_FAIL " + error)
		...
		for action in [&"walk", &"run"]:
			for facing in [Vector3.RIGHT, Vector3.LEFT]:
				...
				for i in [0, 2, 4, 6]:
					...
					get_viewport().get_texture().get_image().save_png("user://painted-%s-%d.png" % [label, i])
	route.player.set_physics_process(true)
