...
func run() -> void:
	...
	var pack: bool=sandbox.is_backpack_enabled()
	await tap("BackpackButton"); check(sandbox.is_backpack_enabled()!=pack,"backpack action once")
	await tap("BackpackButton","mouse"); check(sandbox.is_backpack_enabled()==pack,"mouse backpack once")
	camera_before=cam.rotation; await tap("ViewButton")
	check(not cam.rotation.is_equal_approx(camera_before),"camera quarter turn")
	cam.reset_view(); await settle()
	group("all existing equipment and view actions")

	var p := center("BackpackButton")
	...
	check(sandbox.is_backpack_enabled()!=pack,"small in-button motion remains a tap")
	sandbox.set_backpack_enabled(pack); await settle()
	touch(p,true); drag(outside,11); drag(p,11); touch(p,false); await settle()
	check(sandbox.is_backpack_enabled()==pack,"drag away then return cancels tap")
	sandbox.set_backpack_enabled(pack); await settle()
	touch(p,true); touch(outside,false); await settle()
	check(sandbox.is_backpack_enabled()==pack,"outside release cancels")
	touch(p,true); touch(p,false,11,true); await settle()
	check(sandbox.is_backpack_enabled()==pack,"canceled UP")
	touch(p,true,11,true); touch(p,false); await settle()
	check(sandbox.is_backpack_enabled()==pack,"canceled DOWN")
	mouse(p,true,0,true); mouse(p,false); await settle()
	check(sandbox.is_backpack_enabled()==pack,"canceled mouse DOWN")
	mouse(p,true); mouse_motion(outside); mouse_motion(p); mouse(p,false); await settle()
	check(sandbox.is_backpack_enabled()==pack,"mouse drag away and back cancels")
	sandbox.set_backpack_enabled(pack); await settle()
	...
	check(sandbox.is_backpack_enabled()==pack,"second finger cannot steal")
	mouse(p,true,InputEvent.DEVICE_ID_EMULATION); mouse(p,false,InputEvent.DEVICE_ID_EMULATION)
	check(sandbox.is_backpack_enabled()==pack,"emulated click cannot duplicate pending tap")
	touch(p,false,21); await settle()
	check(sandbox.is_backpack_enabled()!=pack,"owner completes once")
	await tap("BackpackButton"); check(sandbox.is_backpack_enabled()==pack,"ownership freed")
	touch(p,true,0); mouse(p,false); await settle()
	check(sandbox.is_backpack_enabled()==pack,"real mouse cannot release finger zero")
	touch(p,false,0); await settle()
	check(sandbox.is_backpack_enabled()!=pack,"finger zero retains ownership")
	sandbox.set_backpack_enabled(pack); await settle()
	...
	for notification in [NOTIFICATION_APPLICATION_FOCUS_OUT,NOTIFICATION_WM_WINDOW_FOCUS_OUT,NOTIFICATION_APPLICATION_PAUSED]:
		sandbox.set_backpack_enabled(pack); await settle()
		await set_open(true); touch(center("BackpackButton"),true)
		tools_bar.notification(notification); await settle()
		check(not panel.visible,"lost focus folds panel "+str(notification))
		key(KEY_F3); key(KEY_F3,false)
		check(sandbox.is_backpack_enabled()==pack,"inactive shortcut ignored")
		await wake(); touch(p,false); await settle()
		check(not panel.visible and sandbox.is_backpack_enabled()==pack,"late UP cannot activate")
	...
	check(sandbox.is_backpack_enabled()==pack,"tree pause clears gesture")
	sandbox.set_backpack_enabled(pack); await settle()
	...
	key(KEY_F3); key(KEY_F3,false); touch(p,false)
	check(sandbox.is_backpack_enabled()==pack,"menu ignores tools input")
	...
	await set_open(true); p=center("BackpackButton"); await set_open(false)
	...
	check(sandbox.is_backpack_enabled()==pack,"hidden button cannot activate")
	...
	key(KEY_F3); key(KEY_F3,false); check(sandbox.is_backpack_enabled()!=pack,"F3 pack")
	key(KEY_F3,true,true); key(KEY_F3,false); check(sandbox.is_backpack_enabled()!=pack,"echo ignored")
	key(KEY_F3); key(KEY_F3,false)
