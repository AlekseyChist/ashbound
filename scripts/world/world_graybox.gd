...
func _ready() -> void:
	DisplayServer.window_set_title("AshBound — World Graybox 0.21.0")
	...
	for road in layout.roads:
		...
		var strip: MeshInstance3D = shapes.add_strip(points, 6.0, Color("958466"), true)
		strip.name = road.id
		_build_shoulders(points)
	for river in layout.rivers:
		var strip: MeshInstance3D = shapes.add_strip(points_of(river), 12.0, Color("537d91"), false)
		strip.name = river.id
	_build_sites()
	_add_light()
	...
	set_overview(true)
	reset_overview()
	print("WORLD_GRAYBOX_READY version=%s grid=%d roads=5 extent=2000" % [layout.version, layout.width])
...
func _build_shoulders(points: PackedVector3Array) -> void:
...
func _configure_hud() -> void:
	...
	city_picker.custom_minimum_size = Vector2(330, 120)
	city_picker.focus_mode = Control.FOCUS_NONE
	city_picker.auto_translate_mode = Node.AUTO_TRANSLATE_MODE_DISABLED
	for key in CITY_KEYS: city_picker.add_item(Localization.text(key))
	top.add_child(city_picker)
	var popup := city_picker.get_popup()
	# UI book 1.0: roughly 120 logical pixels per touch choice (font + spacing).
	popup.add_theme_constant_override("v_separation", 80)
	popup.add_theme_stylebox_override("panel", root.theme.get_stylebox("panel", "PanelContainer"))
	popup.add_theme_stylebox_override("hover", root.theme.get_stylebox("hover", "Button"))
...
func _refresh_text(_language: String = "") -> void:
	...
	for i in range(CITY_KEYS.size()):
		city_picker.set_item_text(i, Localization.text(CITY_KEYS[i]))
		city_labels[i].text = "%d · %s" % [i + 1, Localization.text(CITY_KEYS[i])]
...
func select_city(index: int) -> void:
	_clear_input()
	selected_city = clampi(index, 0, 3)
	city_picker.select(selected_city)
	var p: Array = layout.cities[selected_city].spawn
	...
	var city: Dictionary = layout.cities[selected_city]
...
func _physics_process(delta: float) -> void:
	...
	for label in city_labels:
		...
		label.pixel_size = overview_distance * 0.00042 if overview else 0.04
