...
SOURCE=ROOT/'docs/design/world-connections-v1/world-connections.json'
...
flat_nodes=[c['point'] for c in cities]+[j['point'] for j in D['junctions']]+[c['plaza'] for c in cities]
for r in roads:
...
for c in cities:
    px,pz,ph=c['plaza'];dist=np.hypot(x-px,z-pz);w=1-smooth((dist-18)/10)
    heights=heights*(1-w)+ph*w
# Actual channel is below water; road strips cross it as decks.
river_weight=1-smooth((water_distance-9)/14)
heights=heights*(1-river_weight)+np.minimum(heights,water_height-2.8)*river_weight
...
road_weight*= (water_distance>20)[:,:,None]
...
layout={'version':'0.21.0-world-graybox','width':WIDTH,'spacing':STEP,'extent_m':2000,
        ...
        'rivers':[{**r,'world_points':dense(r['points'])} for r in D['rivers']],
...
print('WORLD_HEIGHTS_OK grid=401 triangles=320000 roads=5 connectors=3')
