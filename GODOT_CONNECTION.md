...
С 23 сентября 2026 (D-047) локальная Qwen выполняет простые ограниченные поручения. Сложный ввод, взаимодействие нескольких систем и трудные исправления игрового кода/сцен Codex выполняет напрямую; основные тесты, ПК и телефон по-прежнему проверяет Codex. Выбор исполнителя — по [AGENTS.md](AGENTS.md), мост требуется только для задач Ollama.
...
- Новые модели автоматически не скачиваются. Сам мост не переключается на облачного провайдера. Передача трудной реализации Codex разрешена отдельно правилом D-047 и фиксируется в карточке.
