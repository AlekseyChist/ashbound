...
- [Правила работы](AGENTS.md) — Qwen выполняет простые ограниченные задачи; Codex — сложный ввод, связи систем и трудные исправления; основные проверки остаются у Codex.
...
**WASD** — движение относительно камеры, **мышь** — обзор из-за спины, **E** — действие рядом с жителем или поленницей, **ЛКМ** — удар. **Esc** освобождает/захватывает курсор для интерфейса. На Android используются экранные стрелки, «Действие» и «Удар»; жест в свободной правой области вращает камеру. Реплика закрывается нажатием на её панель. «Начать заново» сбрасывает упражнение; прогресс между запусками пока не сохраняется. [Камера и ракурсы героя](docs/levels/THIRD_PERSON_SPRITES.md).



Доступная цель взаимодействия выделяется золотистым, цель тренировочного удара — красноватым. Подсказка называет получателя действия. Обычное касание свободной области экрана не атакует; намеренный удар отдельной кнопкой возможен и без цели. [Подсветка целей](docs/levels/TARGET_FEEDBACK.md), пример собеседника.

[Устройство сцены и места для правок](docs/levels/FIRST_COURTYARD.md) · [Сборки Windows и Android](docs/production/BUILDS.md).

## Стартовая деревня на карте мира

Открыть `scenes/world/world.tscn` и нажать **F6**. Деревня «Лесные дворы» стоит на плато в юго-западной части карты 2×2 км; «Улица» выводит на тропу к лесной таверне, тропа от северного края — к лесной пещере. [Как устроено и что проверено](docs/production/WORLD_VILLAGE_01_TASK.md).
